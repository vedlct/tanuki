...
class Reportm extends CI_Model
{
    ...
    public function filterByCustomer(){
        ...
        $this->db->select('transactionmaster.id as tid,transactionmaster.*, users.name as customer  , paymentType , orderType , memberCardNo, COUNT(transactionmaster.fkOrderId)  as totalorder,COUNT(transactiondetail.fkItemSizeId) as totalitem,  SUM((transactiondetail.quantity*transactiondetail.rate)- transactiondetail.discount) as totalammount  ');
    ...
    public function filterByEmployee(){
        ...
        $this->db->select('users.id as uid,transactionmaster.*, users.name as employee  , paymentType , orderType , memberCardNo, COUNT(transactionmaster.fkOrderId)  as totalorder,COUNT(transactiondetail.fkItemSizeId) as totalitem,  SUM((transactiondetail.quantity*transactiondetail.rate)- transactiondetail.discount) as totalammount  ');
        $this->db->join('transactiondetail', 'transactiondetail.fkTransId = transactionmaster.id ', 'left');
        $this->db->join('orders', 'orders.id = transactionmaster.fkOrderId ', 'left');
    ...
    {
        $this->db->select('items.itemName as itemname, COUNT(items.itemName) as totalitem');
        $this->db->join('itemsizes', 'itemsizes.id  = transactiondetail.fkItemSizeId', 'left');
